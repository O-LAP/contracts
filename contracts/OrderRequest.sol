...
// @title An O-LAP Order Request
contract OrderRequest {
    ...
    event Init(uint256 deadline, int256 destination, uint256 amount, bytes32 message);
    ...
    function OrderRequest(uint256 deadln) public {
        require(now < deadln);
        owner = msg.sender;
        deadline = deadln;
        completed = false;
    ...
    function init(uint amt, int256 dst, bytes32 mssg) public payable {
        require(msg.value >= amt+request_security);
        destination = dst;
        amount = amt;
        message = mssg;
        emit Init(deadline, destination, amount, mssg);
    }
